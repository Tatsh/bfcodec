...
BFCodec::BFCodec(BFCodec&& other) noexcept
...
std::expected<void, BFCodecError> BFCodec::decrypt(std::span<std::byte> data, std::span<const std::byte, 8> iv)
{
	if (!blf_) {
		return std::unexpected(BFCodecError::InvalidCodec);
	}
...
std::expected<void, BFCodecError> BFCodec::encrypt(std::span<std::byte> data, std::span<const std::byte, 8> iv)
{
	if (!blf_) {
		return std::unexpected(BFCodecError::InvalidCodec);
	}
